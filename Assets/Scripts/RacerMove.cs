...
public class RacerMove : MonoBehaviour
{
	...
	public float boostStrength = 150;

	[Header("Hover Settings")]
	public float hoverHeight = 20f;        //The height the ship maintains when hovering
	...
	private InputHandler input;						//A reference to the player's input					
...
//---------------------------------our Variables-----------------------------------
	...
	public bool restoreMaxSpeed = false;
	private Transform UICanvas;
	public Vector3 stopVelocity;
	
	public bool addBoost = false;
	private bool offRoad = false;
	private bool resetTimer = false;
	public bool invertItem = false;
	private float timer = 0;
	public float maxRespawnTimer;

	[Header("Physics Settings")]
	public float slipstreamDistance = 30.0f;		// The max distance for adding slipstream boost
	public float slipStreamFactor = 1.05f;			// Slipstream-boost strength
...
//----------------------------------------------------------------------------------
	...
	void Start()
	{
		...
		UICanvas = this.gameObject.transform.GetChild(2);

		//Calculate the ship's drag value
		drag = driveForce / terminalVelocity;
		timer = maxRespawnTimer;
	}
	...
	void FixedUpdate()
	{
		...
		//---------------------------------our Code-----------------------------------
		// If player is offroad, show UI overlay with countdown
		if (offRoad)
		{
			timer -= Time.deltaTime;
			this.UICanvas.GetChild(5).GetComponent<UnityEngine.UI.Text>().text = ((int)timer+1).ToString();
			if (timer < 0)
			{
				this.UICanvas.GetChild(4).gameObject.SetActive(false);
				this.UICanvas.GetChild(5).gameObject.SetActive(false);
				resetTimer = false;
				timer = maxRespawnTimer;
				GameObject.Find("GameLogic").GetComponent<RaceLogic>().respawnRacer(this.gameObject);
			}
		}
		//----------------------------------------------------------------------------
	}
	...
	void CalculatHover()
	{
		...
		//isOnGround = Physics.SphereCast(transform.position, sphereCastRadius, -(transform.up), out hitInfo, maxGroundDist, whatIsGround);
		...
		if (isOnGround)
		{
			...
			//---------------------------------our Code-----------------------------------
			// Determine if player is currently offroad
			if(hitInfo.collider.gameObject.tag == "offroad")
			{
				offRoad = true;
				this.UICanvas.GetChild(4).gameObject.SetActive(true);
				this.UICanvas.GetChild(5).gameObject.SetActive(true);
			}
			else
			{
				offRoad = false;
		...
		if (invertItem == false)
		{
			angle = angleOfRoll * -input.steering;
		}
		else
		{
			angle = angleOfRoll * input.steering;
		...
		rigidBody.MoveRotation(Quaternion.Lerp(rigidBody.rotation, bodyRotation, Time.deltaTime * 10f));
	...
	void CalculatePropulsion()
	{
		...
		//---------------------------------our Code-----------------------------------
		// Invert steering for driving backwards and for invertItem
		if(speed >= 0f && invertItem == false)
        {
			rotationTorque = input.steering - rigidBody.angularVelocity.y;
        }
        else
        {
			rotationTorque = -input.steering - rigidBody.angularVelocity.y;
		...
		//---------------------------------our Code-----------------------------------
		// Calculating propulsion, taking into account several factors: boost, offroad,slipstream
		if (offRoad)
		{
			rigidBody.velocity *= slowingVelFactor;
		}

		// restore max speed when boost item ended
		if (restoreMaxSpeed)
		{
		...
		bool inSlipstream = isInSlipstream();
		
		if(rigidBody.velocity.magnitude > terminalVelocity)
        {
			propulsion = driveForce * input.acceleration - drag * speed;
        }
        else
        {
			propulsion = driveForce * input.acceleration - drag * Mathf.Clamp(speed, 0f, terminalVelocity);
		}

		if (inSlipstream)
		{
			propulsion *= slipStreamFactor;
		}

		if (addBoost)
		{
			rigidBody.AddForce(transform.forward * boostStrength, ForceMode.Acceleration);
		}
	...
	/**
	 *  -------------------------- OUR CODE -----------------------
	 * Check if the racer is currenly in the slipstream of an opponent
	 */
	private bool isInSlipstream()
	{
		Ray ray = new Ray(transform.position, transform.forward);
		RaycastHit hit;
		bool objectInSight = Physics.Raycast(ray, out hit, slipstreamDistance);

		if (objectInSight && hit.collider.gameObject.tag.Equals("racer"))
		{
			// Calculate angle between the two ships
			float angle = Quaternion.Angle(transform.rotation, hit.collider.gameObject.transform.rotation);
			if (angle < 15.0f) return true;
		}
		return false;
    ...
    private void OnDrawGizmosSelected()
    {
	    ...
	    Debug.DrawLine(transform.position, transform.position + (-transform.up) * hitDistance);
	    Gizmos.DrawWireSphere(transform.position + (-transform.up) * hitDistance, sphereCastRadius);
	    
	    // Windschatten-Raycast
	    Debug.DrawRay(transform.position, transform.forward * slipstreamDistance, Color.green);
